            ...
            stages {
                ...
                stage ('Testing') {
                    ...
                    steps {
                        ...
                        nexusArtifactUploader artifacts: [[artifactId: "${this_stage}_${gitCommit}_data", type: 'xml', classifier: 'testdata', file: "${this_stage}_unittest.xml"],[artifactId: "${this_stage}_${gitCommit}_log", type: 'html', classifier: 'testlog', file: "${this_stage}_unittest_log.xml"],[artifactId: "${this_stage}_${gitCommit}_report", type: 'html', classifier: 'testreport', file: "${this_stage}_unittest_report.xml"]], nexusVersion: 'nexus3', protocol: 'http', nexusUrl: 'nexus:8081/repository/NetCICD-testreports/', groupId :'NetCICD_agent', version: "${gitCommit}", repository: 'NetCICD-reports',credentialsId: 'jenkins-nexus'                   
                    }
                }
                stage ('Cleaning up') {
                    steps {
