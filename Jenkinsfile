            ...
            stages {
                stage ('Collecting variables') {
                    steps {             
                        ...
                        script {
                            cml_token = sh(returnStdout: true, script: 'curl -k -X POST "${env.CML_URL}/api/v0/authenticate" -H  "accept: application/json" -H  "Content-Type: application/json" -d \'{"username":"' + "${CML_CRED_USR}" + '","password":"' + "${CML_CRED_PSW}" + '"}\'').trim()                             
                        }                       
